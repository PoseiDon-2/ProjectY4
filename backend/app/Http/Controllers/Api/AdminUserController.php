    ...
    public function createUser(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $userData = [
                ...
                'phone' => $request->phone,
                // 🌟 จุดแก้ที่ 1: เติม ->value ให้ Enum เพื่อไม่ให้ Database งง
                'role' => $role->value,
                'is_email_verified' => $request->isVerified ?? true,
                // 🌟 จุดแก้ที่ 2: เติม ->value ให้ Enum
                'status' => UserStatus::ACTIVE->value,
                // 🌟 จุดแก้ที่ 3: แปลง interests จาก Array เป็น JSON ป้องกัน Error 500
                'preferred_categories' => !empty($request->interests) ? json_encode($request->interests) : null,
            ...
            if ($request->role === 'organizer') {
                ...
                // 🌟 จุดแก้ที่ 4: สร้างองค์กรใหม่ก่อนเพื่อเอา UUID (แบบเดียวกับที่เราแก้ใน AuthController)
                ...
                // เอา UUID ขององค์กรที่เพิ่งสร้าง มาผูกกับ User (ไม่ใช่เอาเลขมั่วๆ ที่หน้าบ้านส่งมา)
            ...
            return response()->json([
                ...
                'user' => [
                    ...
                    'name' => $user->first_name . ' ' . $user->last_name,
                    'email' => $user->email,
                    'role' => $user->role->value, // ตอนดึงออกก็ต้องมี ->value 
